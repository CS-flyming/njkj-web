        ...
        <Card class="filter-wrap">
            <Form @submit.native.prevent="handleFilter" :model="filter" ref="filterForm" label-position="right" :label-width="120" >
                ...
                <FormItem label="审核状态" prop="auditState">
                    <store-audit-state-selector v-model="filter.auditState" :filter="[-1, 1]"></store-audit-state-selector>
                </FormItem>
                <FormItem label="审核类型" prop="auditAction">
                    <store-audit-type-selector v-model="filter.auditAction"></store-audit-type-selector>
                </FormItem>
...
import storeAuditTypeSelector from "components/store-audit-type-selector";
import pagination from "components/pagination";
import {
    getMerchantStoreAuditList,
    checkMerchantStoreAuditFirst,
    checkMerchantStoreAuditSecond,
    getMerchantStoreAuditFirstInfo
} from "@/actions/store";
export default {
    name: "store-check",
    data() {
    ...
    methods: {
        loadData() {
            this.loading = true;
            getMerchantStoreAuditList(this.filter).then(
                res => {
                    this.loading = false;
        ...
        firstCheckSubmit() {
            this.$refs.firstModalCheckForm.validate(valid => {
                if (valid) {
                    ...
                    checkMerchantStoreAuditFirst(formData).then(
                        res => {
