        ...
        <Card class="filter-wrap">
          <!-- filterForm  引用ref重置 -->
            <Form @submit.native.prevent="handleFilter" :model="filter" ref="filterForm" label-position="right" :label-width="120" >
                <FormItem label="通道编号" prop="institutionProductCode">
                    <Input v-model="filter.institutionProductCode"></Input>
                </FormItem>
                ...
                <FormItem label="通道名称" prop="code">
                    <channel-selector v-model="filter.code"></channel-selector>
                </FormItem>
...
import { getInstitutionDailyCharge } from "@/actions/statictics";
export default {
    ...
    data() {
        return {
            ...
            filter: {
                ...
                institutionProductCode: "", //定义编号为code类型
                beginDate: "",
                endDate: "" //两个都是时间控件相关
                // salesmanId: "",
                // auditState: "",
    ...
    methods: {
        loadData() {
            this.loading = true;
            getInstitutionDailyCharge(this.filter).then(
                res => {
                    this.loading = false;
        ...
        downloadExcel() {
            this.$util.downloadExcel(
                "settle/charge/daily/collect/export/institution",
                this.filter
            );
        }
    },
